...
function CourseDetails() {
  ...
  useEffect(() => {
    const fetchCourseAndTeacher = async () => {
      try {
        const res = await axios.get(`https://learn-hub-backend-2bhg.onrender.com/courses/${id}`);
        setCourse(res.data);

        const teacherRes = await axios.get(`https://learn-hub-backend-2bhg.onrender.com/teacher/${res.data.teacherId}`);
